...
class KMeansClustering:
    ...
    def e_step(self, data):
        ...
        """
        Cluster_assignment_of_all = []
        for i in range(data.shape[0]):
            
            distance = np.linalg.norm(data[i] - self.centroids, axis=1)
            Cluster_assignment_of_all.append(np.argmin(distance))
            
        return np.array(Cluster_assignment_of_all)
        pass
    ...
    def evaluate(self, data, cluster_assign):
        ...
        metric = 0.0
        for i in range(data.shape[0]):
            metric += np.power(np.linalg.norm(data[i] - self.centroids[cluster_assign[i]]),2)
